...
void CommandRegistry::register_all_commands(CommandProcessor& processor) {
    ...
    processor.register_command("add", Constants::ADD_RESIDENT_ARGUMENTS ,new AddResidentCommand(manager, sim));
    processor.register_command("remove", Constants::REMOVE_RESIDENT_ARGUMENTS , new RemoveResidentCommand(manager, sim));
    ...
    processor.register_command("info", Constants::RESIDENT_INFO_ARGUMENTS, new ResidentInfoCommand(manager));
